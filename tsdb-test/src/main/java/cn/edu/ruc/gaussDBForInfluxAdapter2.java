...
public class gaussDBForInfluxAdapter2 implements BaseAdapter {// ctrl+i 快速实现接口
    ...
    public long query1(long start, long end) {
        String sqlFormat = "select time,f,d,%s from sensor where f='%s' and d='%s' and time>=%s" +
                "and time<=%s";
        String eSql = String.format(sqlFormat, "s1","f1","d2", TimeUnit.MILLISECONDS.toNanos(start),TimeUnit.MILLISECONDS.toNanos(end));
        return execQuery(eSql);
    }

    public long query2(long start, long end, double value) {
        String sqlFormat = "select time,f,d,%s from sensor where f='%s' and s1>%s and time>=%s and time< %s";
        String eSql = String.format(sqlFormat, "s1", "f1", value, TimeUnit.MILLISECONDS.toNanos(start), TimeUnit.MILLISECONDS.toNanos(end));
        return execQuery(eSql);
    }

    public long query3(long start, long end) {
        String sqlFormat = "select mean(*) from sensor where f='%s' and time>=%s and time<=%s group by " +
                "f,d,time(1h)";
    ...
    public long query4(long start, long end) {
        String sqlFormat = "select time,f,d,%s,%s,%s,%s,%s from sensor " +
                "where f='%s' and  time>=%s and time<%s";
        String eSql = String.format(sqlFormat, "s1", "s2", "s3", "s4", "s5", "f1", TimeUnit.MILLISECONDS.toNanos(start), TimeUnit.MILLISECONDS.toNanos(end));
        return execQuery(eSql);
    }
    ...
    */
    public long insertData(String data) {
        ...
        for (String row : rows) {
            ...
            Point.Builder builder = Point.measurement("sensor")
